...
RSpec.describe BulkDiscount, type: :model do
  ...
  describe 'class methods' do
    describe "::order_by_quantity_threshold" do
      it 'returns discounts in order from highest quantity to lowest' do
        expected = [@bulk_discount_1, @bulk_discount_3, @bulk_discount_2, @bulk_discount_4]

        expect(BulkDiscount.order_by_quantity_threshold).to eq(expected)
      end
    end
  end
end
